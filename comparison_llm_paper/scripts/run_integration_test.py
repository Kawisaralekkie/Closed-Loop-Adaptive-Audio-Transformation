#!/usr/bin/env python3
"""Integration test script — run both pipelines with a user-provided audio file.

Usage:
    python scripts/run_integration_test.py <audio_file_path>

Supports WAV, M4A, MP3, FLAC, OGG, etc. Non-WAV files are auto-converted
to WAV PCM mono 16 kHz using ffmpeg (must be installed).

Tests:
  1. Fixed Baseline Pipeline — privacy_target="high"  (MidBandAttenuation)
  2. Fixed Baseline Pipeline — privacy_target="very_high" (StrongBlurring)
  3. Agentic Pipeline        — privacy_target="high"
  4. Agentic Pipeline        — privacy_target="very_high"

Each run prints per-chunk privacy_score, preserve_score, classification
top-3, routing decision, and overall pass/fail.
"""
...
def build_mock_kb(privacy_target: str) -> KnowledgeBase:
    """Build a local KnowledgeBase (no S3 needed) for integration testing."""
...
def print_run_report(report, label: str) -> None:
    ...
    for cr in report.chunks:
        ...
        if cr.metrics:
            ...
            print(f"     utility:        mAP={um.mAP:.4f}, F1={um.f1:.4f}, accuracy={um.accuracy:.4f}")
...
def save_run_log(all_runs: list[dict], audio_path: str, max_chunks: int | None) -> str:
    """Save all pipeline run results to a timestamped JSON log file.
...
# ── Shared tool instances (loaded once) ──────────────────────────────────

_tools_cache: dict = {}


def get_shared_tools(output_dir: str) -> dict:
    """Lazily initialize and cache heavy tool instances (models load once)."""
    if _tools_cache:
        return _tools_cache

    print("Loading models (Silero VAD + YAMNet + Whisper + ECAPA-TDNN) — this may take a moment...")
...
def run_fixed_baseline(wav_path: str, privacy_target: str, output_dir: str):
    ...
    print(f"\n>>> Starting Fixed Baseline — privacy_target={privacy_target!r} ...", flush=True)
    report = pipeline.run(request)
    label = f"Fixed Baseline — privacy_target={privacy_target!r}"
    print_run_report(report, label)
    return report, label


def run_agentic(wav_path: str, privacy_target: str, output_dir: str):
    tools = get_shared_tools(output_dir)
    kb = build_mock_kb(privacy_target)

    agent = AdaptivePrivacyControlAgent(
        mid_band_tool=tools["mid_band"],
        strong_blur_tool=tools["strong_blur"],
        quality_tool=tools["quality"],
    )
    ...
    print(f"\n>>> Starting Agentic Pipeline — privacy_target={privacy_target!r} ...", flush=True)
    report = pipeline.run(request)
    label = f"Agentic Pipeline — privacy_target={privacy_target!r}"
    print_run_report(report, label)
    return report, label
...
def main() -> None:
    if len(sys.argv) < 2:
        print("Usage: python scripts/run_integration_test.py <audio_file> [--max-chunks N]")
        sys.exit(1)
    ...
    output_dir = tempfile.mkdtemp(prefix="integration_test_")
    ...
    # Optionally trim audio to limit chunks for faster testing
    ...
    # ── 1. Fixed Baseline — high (MidBandAttenuation) ──
    all_runs: list[dict] = []

    report, label = run_fixed_baseline(wav_path, "high", output_dir)
    all_runs.append(report_to_dict(report, label))

    # ── 2. Fixed Baseline — very_high (StrongBlurring) ──
    report, label = run_fixed_baseline(wav_path, "very_high", output_dir)
    all_runs.append(report_to_dict(report, label))

    # ── 3. Agentic Pipeline — high ──
    report, label = run_agentic(wav_path, "high", output_dir)
    all_runs.append(report_to_dict(report, label))

    # ── 4. Agentic Pipeline — very_high ──
    report, label = run_agentic(wav_path, "very_high", output_dir)
    all_runs.append(report_to_dict(report, label))
