...
TOOL_DEFINITIONS = [
    ...
    # NOTE: apply_source_separation_blur toolSpec REMOVED —
    # source separation is globally disabled. The LLM can no longer select it.
]
...
## Decision Guidelines
- Start with lighter techniques (midband) and escalate only if needed.
...
- Source separation takes more processing time — prefer frequency blurring when time constraints are critical, source separation when preserve_score optimization is prioritized.
...
class LLMPrivacyControlAgent:
    ...
    def __init__(
        ...
        # SS is hard-disabled across the whole system. Ignore any tool passed
        # in so the LLM cannot select source separation as a recipe.
